...
if(file.exists(train_file)){
    ...
    download.file(URL_train,train_file,method="curl")
    dat <- read.csv("pml_train.csv")
}
...
##Cleaning the Data
I have removed columns containing either no data or a large number of NA values. I have also removed the first 7 columns which contained the individuals data as I am not interested in this here. This leaves 49 variables (including the classe). These are gyropscope, acceleration and magnetometer data in x, y, z, as well as the roll, pitch and yaw for each of the belt, forearm, arm and dumbbell.
...
```
This algorithm has a much better accurancy of 99.6%.
...
##Summary
This dataset consisted of the data from sensors (acceleratometer, gyroscope, and magnetometer) on 4 body areas (belt, forearm, arm and dumbbell) while a dumbbell was lifted in the correct way and 4 incorrect ways.

The data was cleaned to remove spurious rows and columns with a large nmber of missing values. Three machine learning algorithms were applied to the training dataset. 
Using a random forest algorithm, the test data set was classified with a 99.6% accuracy. When applied to the quix data set, 20 out of 20 of the objects were correctly classified.
